...
class Post extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    {
        $this->_init('field_blog_post', 'post_id');
    }
    ...
    protected function _beforeDelete(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        $this->getConnection()->delete($this->getTable('field_blog_post_store'), $condition);
    ...
    {   
        ...
        // STORE
            if($stores = $object->getStores()){
                $table = $this->getTable('field_blog_post_store');
        ...
        // Categories
            if($newCategories = (array)$object->getCategories()){
                $table = $this->getTable('field_blog_post_category');
                $where = ['post_id = ?' => (int)$object->getId()];
                $this->getConnection()->delete($table, $where);
                $data = [];
        ...
        // Posts Related
            if($postsRelated = $object->getPostsRelated()){
                $table = $this->getTable('field_blog_post_related');
                $where = ['post_id = ?' => (int)$object->getId()];
                $this->getConnection()->delete($table, $where);
                $data = [];
        ...
        // Posts Tag
            if($tags = $object->getTags()){
                $tags = explode(",", $tags);
                if(!empty($tags)){
                    $table = $this->getTable('field_blog_post_tag');
                    $where = ['post_id = ?' => (int)$object->getId()];
                    $this->getConnection()->delete($table, $where);
                    $data = [];
    ...
    protected function _afterLoad(\Magento\Framework\Model\AbstractModel $object)
    {
        if ($object->getId()) {
            ...
            if ($id = $object->getId()) {
                ...
                ->from($this->getTable('field_blog_post_related'))
                ->where(
                    'post_id = '.(int)$id
                    );
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                ['cbs' => $this->getTable('field_blog_post_store')],
    ...
    public function lookupStoreIds($id)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('field_blog_post_store'),
            'store_id'
            )->where(
            'post_id = :post_id'
            );
    ...
    public function lookupPostsRelatedIds($id)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('field_blog_post_related'),
            'post_related_id'
            );
    ...
    public function lookupCategoryIds($id)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('field_blog_post_category'),
            'category_id'
            )->where(
            'post_id = :post_id'
            );
