...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getPostCategories($postId){
        if($this->_postCategories == NULL){
            ...
            $select = 'SELECT * FROM ' . $this->_resource->getTableName('field_blog_post_category') . ' WHERE post_id = ' . $postId . ' ORDER BY position ASC';
    ...
    public function getPostCategoryUrl($post)
    {
        ...
        if(!empty($categories)){
            $connection = $this->_resource->getConnection();
            $select = 'SELECT * FROM ' . $this->_resource->getTableName('field_blog_category') . ' WHERE category_id = ' . $categories[0]['category_id'];
