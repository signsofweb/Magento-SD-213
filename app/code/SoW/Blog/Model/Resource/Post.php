...
namespace SoW\Blog\Model\Resource;
...
class Post extends AbstractDb
{
    ...
    {
        $this->_init('sow_blog_post', 'post_id');
    }

    protected function _beforeDelete(AbstractModel $object)
    {
        $condition = ['post_id = ?' => (int)$object->getId()];
        $this->getConnection()->delete($this->getTable('sow_blog_comment'), $condition);
        $this->getConnection()->delete($this->getTable('sow_blog_post_store'), $condition);
    ...
    protected function _afterSave(AbstractModel $object)
    {
        ...
        $table = $this->getTable('sow_blog_category_post');
        ...
        $table = $this->getTable('sow_blog_post_store');
    ...
    protected function _getLoadSelect($field, $value, $object)
    {
        ...
        if ($object->getStoreId()) {
            ...
            $select->join(
                ['sow_blog_post_store' => $this->getTable('sow_blog_post_store')],
                $this->getMainTable() . '.post_id = sow_blog_post_store.post_id',
                []
            )->where(
                'status = ?',
                1
            )->where(
                'sow_blog_post_store.store_id IN (?)',
                $storeIds
            )->order(
                'sow_blog_post_store.store_id DESC'
            )->limit(
                1
            );
        }
    ...
    protected function _getLoadByIdentifierSelect($identifier, $store, $isActive = null)
    {
        $select = $this->getConnection()->select()->from(
            ...
            ['cps' => $this->getTable('sow_blog_post_store')],
    ...
    public function lookupStoreIds($postId)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('sow_blog_post_store'),
            'store_id'
        )->where(
            'post_id = ?',
            (int)$postId
    ...
    public function lookupCategories($postId)
    {
        $connection = $this->getConnection();
        $select = $connection->select()->from(
            $this->getTable('sow_blog_category_post'),
            'category_id'
        )->where(
            'post_id = ?',
            (int)$postId
